...
import threading
gpsDict = { 'lon' : 0, 'lat' : 0, 'alt' : 0, 'status' : 0 }
...
#        print(self.user)
        ...
        self.socket=None
        self.stream = serial.Serial('/dev/ttyS0', 38400, timeout=3)
                        ...
                        try:
                            ...
                            if bytes("GNGGA",'ascii') in raw_data :
                                ...
                                if (len(tmp) >= 15 and tmp[0] == '$GNGGA'):# Got GNGGA data
                                    ...
                                    '''
                                    lat = float(tmp[2][:2]) + float(tmp[2][2:]) / 60# DMM to DD
                                    lon = float(tmp[4][:3]) + float(tmp[4][3:]) / 60# DMM to DD
                                    if (tmp[3] == 'S'):
                                        lat = -lat
                                    if (tmp[5] == 'W'):
                                        lon = -lon
...
if __name__ == '__main__':
    ...
    parser.add_option("-t", "--latitude", type="float", dest="lat", default=24.997907, help="Your latitude.  Default: %default")
    parser.add_option("-g", "--longitude", type="float", dest="lon", default=121.323397, help="Your longitude.  Default: %default")
    parser.add_option("-e", "--height", type="float", dest="height", default=92, help="Your ellipsoid height.  Default: %default")
